...
podTemplate(
  ...
  container('node') {
      try{
                ...
                sh label: 'generating report', script: 'node reporter.js'
      }
      catch(err){
          
      }
      finally{
          publishHTML([allowMissing: false, alwaysLinkToLastBuild: false, keepAll: false, reportDir: 'report/report.html', reportFiles: 'report.html', reportName: 'HTML Report', reportTitles: ''])
      }
    }
  }
}